...
function send(response, status, body, headers = {}) {
  ...
  response.writeHead(status, {
    ...
    'access-control-allow-headers':
      'Content-Type, Authorization, X-Course-Actor, X-Course-Scenario, Idempotency-Key',
    'content-type':
      typeof body === 'string'
        ? 'application/json'
...
async function readJson(request) {
  ...
  for await (const chunk of request) {
    ...
    if (bytes > 131072) {
      throw new Error('request too large for teaching fixture');
    }
...
const server = createServer(async (request, response) => {
  ...
  if (
    url.pathname.startsWith('/v1/incidents') ||
    url.pathname === '/v1/geocoding' ||
    url.pathname === '/v1/session/login'
  ) {
    try {
      return await handleCampusOps(request, response, url, {
        send,
        readJson,
        scenario,
      });
  ...
  if (request.method === 'GET' && url.pathname === '/v1/resources') {
    ...
    if (scenario === 'server_error') {
      return send(response, 500, { code: 'controlled_failure' });
    }
    ...
    const payload =
      scenario === 'nullable'
        ? null
        : {
            label: 'CampusOps: incidencia sintética',
  ...
  if (request.method === 'POST' && url.pathname === '/v1/resources/action') {
    ...
    if (!input || typeof input.resourceId !== 'string') {
    ...
    if (scenario === 'timeout_after_commit') {
      return setTimeout(() => send(response, 200, result), 1500);
    }
...
server.listen(port, host, () => {
  ...
  process.stdout.write(
    `DMI controlled backend listening at http://${host}:${boundPort}\n`,
  );
});