# Semana 3 — ¿Cómo se califica la actividad?
...
|---|---|---:|
| AC-01 — Reproducción | La versión identificada por el SHA se instala y pasa `make verify-week-03`; se conserva el reporte. | 2.5 |
| AC-02 — Comportamiento | El workflow ejecuta las comprobaciones obligatorias con mínimo privilegio y conserva resultados; el modelo vincula activos, amenazas, controles y pruebas. Se comprueba con pruebas públicas y reportes. | 2.0 |
| AC-03 — Falla | Demuestren que un fallo obligatorio no se oculta y que los controles del modelo se pueden comprobar, en vez de quedar sólo escritos. Conserven comandos y resultados observables. | 1.5 |
| AC-04 — Decisión | Justifiquen qué amenaza atienden primero y por qué el control seleccionado reduce su riesgo. Relaciónenlo con pruebas en `evidence/week-03/engineering.json`. | 1.5 |
| AC-05 — Aportación individual | Cada integrante tiene una aportación verificable en `evidence/week-03/individual.json` y puede explicarla cuando se solicite. | 0.5 |
